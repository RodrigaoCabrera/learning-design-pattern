...
describe("catalog", () => {
  ...
  it("marks only implemented patterns as available", () => {
    ...
    expect(available).toEqual(["observer", "singleton", "strategy", "decorator", "factory-method", "adapter"]);
  });
});
...
describe("lessons registry", () => {
  ...
  it("exposes all implemented lessons", () => {
    expect(allLessons().map((l) => l.slug)).toEqual(["observer", "singleton", "strategy", "decorator", "factory-method", "adapter"]);
  });
});
